...
export const exportToExcelCCM = async (doc) => {
  ...
  const filename = FileSystem.documentDirectory + "Registro CMM-" + doc.date + "-" + doc.id + ".xlsx";
...
export const exportToExcelTD = async (doc) => {
  ...
  const data = [
    ['Relacion de Interruptores Instalados en TGD o TD'],
    ['NOMBRE DEL TABLERO', doc.nameTablero, 'ALIMENTADOR', doc.corrNom],
    ['ID', doc.idTGDTab, 'PROTECCION', doc.protectionFuen],
    ['Nombre E ID', 'NOMBRE DEL TABLERO', 'ID DEL TABLERO',
      ...
      'NO. CABLES TIERRA', 'CALIBRE CABLES TIERRA', 'MAT CABLES TIERRA',
  ...
  const base64 = XLSX.write(wb, { type: "base64" });
  const filename = FileSystem.documentDirectory + "Registro TD-" + doc.date + "-" + doc.id + ".xlsx";
  FileSystem.writeAsStringAsync(filename, base64, {
    encoding: FileSystem.EncodingType.Base64
  }).then(() => {
    Sharing.shareAsync(filename);
...
export const exportToExcelTGD = async (doc) => {
  const data = [
    ...
    ["Nombre" ,"ID", 'INTERRUPTOR', 'TENSIÓN (VOLTS)', 'ICC(KA)', 'NO. POLOS', 'FUSIBLES',
     ...
     'NO. CABLES TIERRA', 'CALIBRE CABLES TIERRA', 'MAT CABLES TIERRA', 'LONGITUD (m)', 'FECHA'],
    [
      doc.name,
      doc.idTGDTab,
      doc.interruptor,
